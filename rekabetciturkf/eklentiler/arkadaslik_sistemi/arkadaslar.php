...
$sor = "select * from $tablo_arkadaslar where kullanici_adi = '$kullanici'";
@$cevap = $vt->query($sor);
...
for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
    if($sayfa == $s) {
        $sayfalama .= '<font color="black"> [' . $s . '] </font>';
    } else {
...
$sayfalama .= '</br>';
// Sayfalama sonu
$yazdir = "select * from $tablo_arkadaslar where kullanici_adi = '$kullanici' order by id DESC LIMIT $baslangic,$limit";
@$vtsonuc = $vt->query($yazdir);
@$a_sayi = $vt->num_rows($vtsonuc);
// Arkadaş ekleme bildirimi varsa okundu olarak işaretleniyor
$vtsorgub = "UPDATE $tablo_bildirimler SET okundu='1' WHERE uye_id='$kullanici_kim[id]' AND tip='21' AND okundu='0' ORDER BY id LIMIT 1";
...
$renkver = 1;
$sayfa_adi = 'Arkadaş listesi';
include_once('bilesenler/sayfa_baslik.php');

//	TEMA UYGULANIYOR	//

$ornek1 = new phpkf_tema();
$tema_dosyasi = 'eklentiler/arkadaslik_sistemi/tema_dosyasi/arkadaslar.php';
...
$ornek1->dongusuz(array('{ARKADAS_SAYI}' => $satirsayisi,
'{SAYFALAMA}' => $sayfalama));
